...
cmd(
  ...
  async (robin, mek, m, { q, reply }) => {
    try {
      ...
      if (!res?.status || !res?.data?.url) {
        return reply("❌ *😓 මම JID අයින් කලා සෙම්ඩ් වෙන්නැත්තෙ එඑකයි 😌✌️. Capada🥲?*");
      }
...
ʳᵉᵃᶜᵗ       ˢᵃᵛᵉ       ˢʰᵃʳᵉ`;
      ...
      await robin.sendMessage(
        mek.key.remoteJid,
        {
          text: `*😓 මම JID අයින් කලා සෙම්ඩ් වෙන්නැත්තෙ එඑකයි 😌✌️ Capada?.*`,
        },
        { quoted: mek }
      );
    } catch (e) {
      console.error(e);
      reply("*😓 මම JID අයින් කලා සෙම්ඩ් වෙන්නැත්තෙ එඑකයි 😌✌️ Capada?.*");
    }
  }
);
cmd(
